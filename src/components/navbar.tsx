  ...
  return (
    <MTNavbar
      ...
      <div className="flex items-center justify-between px-6 lg:px-12 py-0">
        ...
        {/* Desktop menu */}
        <ul className="hidden lg:flex gap-5 font-semibold text-sm items-center">
          {navItems.map((item) => (
            <li key={item.id}>
              <a href={item.href} className="transition hover:text-[#58a8ab]">
                {item.label}
              </a>
            </li>
          ))}
      ...
      <Collapse open={open}>
        <div className="container mx-auto mt-3 rounded-2xl bg-white px-6 py-3">
          <ul className="flex flex-col gap-2 text-gray-900 font-semibold text-sm">
            {navItems.map((item) => (
              <li key={item.id}>
                <a href={item.href} className="block transition hover:text-[#58a8ab]">
                  {item.label}
                </a>
              </li>
            ))}
          ...
          <div className="mt-3 flex gap-2">
            {["twitter", "facebook", "instagram"].map((icon) => (
              <IconButton key={icon} variant="text" color="gray" size="sm"  placeholder={undefined} onResize={undefined} onResizeCapture={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
                <i className={`fa-brands fa-${icon} text-sm`} />
              </IconButton>
            ))}
            <Button color="gray" size="sm" className="ml-auto px-3 py-1 text-sm"  placeholder={undefined} onResize={undefined} onResizeCapture={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
              Contato
            </Button>
          </div>
        </div>
