...
export default function ResultadosSection() {
  ...
  >
    <Image src={icon} alt={`sol-${i}`} width={50} height={50} className="mb-2" />
    ...
    {i === 0 ? (
      <Link href="/page2" scroll={false} onClick={() => window.scrollTo(0,0)} className="w-full mt-auto flex ">
        <button
          className="w-full flex items-center justify-start gap-3 rounded-full bg-[#58A8AB] py-1 px-2 text-white font-semibold text-sm whitespace-nowrap shadow-[0_80px_30px_0_rgba(88,168,171,0.05)] text-xs"
          style={{ fontFamily: "TT Commons Pro" }}
        >
          <span>
            <div className="flex w-[20px] h-[20px] items-center justify-center p-1 bg-[#fafdff] rounded-full">
              <Image alt="Arrow" src="/image/arrow.png" width={10} height={10} />
            </div>
          </span>
          {buttonText}
        </button>
