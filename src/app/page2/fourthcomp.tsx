...
function KPIItem({ icon, value, subtitle, index }: { icon: string; value: string; subtitle: string; index: number }) {
    ...
    <section className="relative">
      <div className="relative z-10 bg-[#E0E0E0] py-16 px-6 md:py-20 md:px-10">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-x-6 gap-y-2">
          {KPIS.map((kpi, i) => (
            <KPIItem key={i} icon={kpi.icon} value={kpi.value} subtitle={kpi.subtitle} index={i} />
          ))}
        </div>
      </div>
    </section>
